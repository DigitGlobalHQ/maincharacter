...
function _renderDossier(doc, auditId, report, photoBuffer) {
  ...
  // ─── 01 · Facial Metrics (geometric KPIs measured from the capture) ───
  const fm = report.faceMeasured;
  if (fm) {
    startSection(300);
    sectionHead('01', 'MEASURED FROM YOUR CAPTURE', 'The Facial Metrics',
      'The geometric read behind the scores: symmetry, proportion, and structure, measured from your photograph. Context for everything that follows.');
    doc.y += 6;
    const stats = [];
    if (fm.attractiveness && typeof fm.attractiveness.score === 'number') stats.push(['COMPOSITE', fm.attractiveness.score + ' / 100', 'Overall facial harmony']);
    if (fm.symmetry && typeof fm.symmetry.overall === 'number') stats.push(['SYMMETRY', fm.symmetry.overall + ' / 100', 'Eyes ' + (fm.symmetry.eyes != null ? fm.symmetry.eyes : '–') + ' · Jaw ' + (fm.symmetry.jaw != null ? fm.symmetry.jaw : '–')]);
    if (fm.canthalTilt && fm.canthalTilt.degrees != null) stats.push(['CANTHAL TILT', (fm.canthalTilt.degrees > 0 ? '+' : '') + fm.canthalTilt.degrees + '°', fm.canthalTilt.label || '']);
    if (fm.jawline && typeof fm.jawline.score === 'number') stats.push(['JAWLINE', fm.jawline.score + ' / 100', 'Gonial ' + (fm.jawline.gonialAngle != null ? fm.jawline.gonialAngle + '°' : '–')]);
    if (fm.goldenRatio && typeof fm.goldenRatio.score === 'number') stats.push(['GOLDEN RATIO', fm.goldenRatio.score + ' / 100', 'Proximity to 1.618']);
    if (fm.faceShape && fm.faceShape.shape) stats.push(['FACE SHAPE', cap(fm.faceShape.shape), fm.faceShape.confidence != null ? fm.faceShape.confidence + '% confidence' : 'Geometric read']);
    const per = 3, g = 12, cw = (W - g * (per - 1)) / per, chh = 62;
    let rt = doc.y;
    stats.forEach((st, i) => {
      const col = i % per;
      if (col === 0 && i > 0) rt += chh + g;
      if (col === 0 && rt + chh > BOTTOM) { doc.addPage(); doc.x = M; doc.y = M + 6; rt = doc.y; }
      const sx = M + (cw + g) * col, sy = rt;
      doc.roundedRect(sx, sy, cw, chh, 9).fill(CARD);
      doc.roundedRect(sx, sy, cw, chh, 9).lineWidth(1).strokeColor(HAIR).stroke();
      eyebrow(st[0], sx + 13, sy + 12, FAINT, 6.5);
      doc.font(SERIF_B).fontSize(18).fillColor(CREAM).text(st[1], sx + 13, sy + 23, { width: cw - 26, lineBreak: false });
      if (st[2]) doc.font(SANS).fontSize(7).fillColor(DIM).text(st[2], sx + 13, sy + 46, { width: cw - 26, lineBreak: false });
    });
    doc.y = rt + chh + 12;
    if (Array.isArray(fm.facialRatios) && fm.facialRatios.length) {
      need(36); eyebrow('FACIAL RATIOS', M, doc.y, FAINT, 7); doc.y += 13;
      const rl = fm.facialRatios.map((r) => (r.name || '') + ' ' + r.value + (r.ideal ? '  (ideal ' + r.ideal + ')' : '')).join('      ');
      doc.font(MONO).fontSize(8).fillColor(SILVER).text(rl, M, doc.y, { width: W, lineGap: 3 });
      doc.y += 6;
    }
  }

  startSection(420);
  sectionHead('02', 'THE READING', 'Where Presence Is Leaking',
  ...
  if (fixed.length) {
    startSection(220);
    sectionHead('03', 'READ AS CONTEXT, NOT GRADED', 'Your Fixed Architecture',
  ...
  if (c) {
    startSection(300);
    sectionHead('04', 'THE LEVER WITH NO EFFORT', 'The Chromatic Arsenal',
    ...
    // ── The Cosmetic Arsenal: lip wardrobe + complexion directives ──
    const cos = c.cosmetic;
    if (cos && ((Array.isArray(cos.lipWardrobe) && cos.lipWardrobe.length) || (Array.isArray(cos.complexion) && cos.complexion.length))) {
      if (Array.isArray(cos.lipWardrobe) && cos.lipWardrobe.length) {
        need(40); doc.y += 6;
        eyebrow('THE COSMETIC ARSENAL  ·  THE LIP WARDROBE', M, doc.y, FAINT, 7.5); doc.y += 16;
        const per = 3, pgap = 12, lcW = (W - pgap * (per - 1)) / per;
        const lips = cos.lipWardrobe.slice(0, 6);
        let rowTop = doc.y;
        for (let r = 0; r < lips.length; r += per) {
          const row = lips.slice(r, r + per);
          let noteH = 0;
          row.forEach((s) => { if (s.note) { doc.font(SANS).fontSize(7.5); noteH = Math.max(noteH, doc.heightOfString(s.note, { width: lcW - 24, lineGap: 1.4 })); } });
          const lcH = 78 + noteH;
          if (rowTop + lcH > BOTTOM) { doc.addPage(); doc.x = M; doc.y = M + 6; rowTop = doc.y; }
          row.forEach((s, ci) => {
            const sx = M + (lcW + pgap) * ci, sy = rowTop;
            doc.roundedRect(sx, sy, lcW, lcH, 9).fill(CARD);
            doc.roundedRect(sx, sy, lcW, lcH, 9).lineWidth(1).strokeColor(HAIR).stroke();
            doc.save(); doc.roundedRect(sx, sy, lcW, 40, 9).clip(); doc.rect(sx, sy, lcW, 40).fill(safeHex(s.hex)); doc.restore();
            doc.font(SERIF).fontSize(11).fillColor(CREAM).text(s.name || '', sx + 12, sy + 50, { continued: true, lineBreak: false })
               .font(MONO).fontSize(7).fillColor(FAINT).text('   ' + safeHex(s.hex).toUpperCase(), { lineBreak: false });
            if (s.note) doc.font(SANS).fontSize(7.5).fillColor(DIM).text(s.note, sx + 12, sy + 66, { width: lcW - 24, lineGap: 1.4 });
          });
          rowTop += lcH + pgap;
        }
        doc.y = rowTop - pgap + 16;
      }
      if (Array.isArray(cos.complexion) && cos.complexion.length) {
        need(40); eyebrow('COMPLEXION  ·  CHEEK  ·  EYE', M, doc.y, FAINT, 7.5); doc.y += 14;
        cos.complexion.forEach((it) => {
          need(28);
          doc.font(SERIF).fontSize(10).fillColor(CREAM).text(it.area || '', M, doc.y, { width: W });
          doc.font(SANS).fontSize(8.5).fillColor(DIM).text(it.directive || '', M + 12, doc.y + 1, { width: W - 12, lineGap: 1.5 });
          doc.y += 8;
        });
      }
      doc.font(SANS_I).fontSize(7).fillColor(FAINT).text('Cosmetic guidance is calibrated to colour biology only and is independent of gender expression.', M, doc.y + 2, { width: W }); doc.y += 8;
    }
  }
  ...
  if (iv) {
    startSection(220);
    sectionHead('05', 'THE WORK', 'The 90-Day Protocol',
    ...
    const stepH = (s) => {
      ...
      let aH = doc.heightOfString((s.agent || s.step || '') + (s.rx ? '  [Rx]' : ''), { width: AGENT_W });
      if (s.spec) { doc.font(SANS_I).fontSize(7.5); aH += doc.heightOfString(s.spec, { width: AGENT_W }) + 2; }
      doc.font(SANS).fontSize(8.5);
      const dH = doc.heightOfString(s.rationale || '', { width: DESC_W, lineGap: 1.5 });
    ...
    blocks.forEach(([title, sub, steps]) => {
      ...
      steps.forEach((s, i) => {
        ...
        if (s.spec) doc.font(SANS_I).fontSize(7.5).fillColor(FAINT).text(s.spec, bx + 52, doc.y + 1, { width: AGENT_W });
        doc.font(SANS).fontSize(8.5).fillColor(DIM).text(s.rationale || '', bx + DESC_X, sy, { width: DESC_W, lineGap: 1.5 });
        sy += sh;
      });
      doc.y = by + h + 14;
    });
  ...
  if (proj) {
    startSection(260);
    sectionHead('06', 'THE HORIZON', 'What Ninety Days Produces',
  ...
  // ════════════════════ THE PROGRAMME ════════════════════
  startSection(260);
  sectionHead('07', 'THE PROGRAMME', 'How This Continues',
