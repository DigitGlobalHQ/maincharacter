...
// lift() raises the toggle above any bottom-pinned fixed/sticky bar (PWA bottom
// nav `.nav`, the audit sticky unlock CTA) so it never overlaps them. It measures
// at runtime, so it ignores TOP navs (also `.nav`) and hidden/zero-height CTAs.
const TOGGLE = `<script>document.addEventListener('DOMContentLoaded',function(){try{var k='mc-theme',r=document.documentElement,b=document.createElement('button');b.className='mc-theme-toggle';b.type='button';function cur(){return r.getAttribute('data-theme')==='light'?'light':'dark'}function paint(){var l=cur()==='light';b.textContent=l?'\\u25D1':'\\u25D0';var t=l?'Switch to dark mode':'Switch to light mode';b.setAttribute('aria-label',t);b.title=t}function lift(){try{var vh=window.innerHeight,max=0;document.querySelectorAll('.nav,.lm-sticky-unlock,.sticky-cta,[data-bottom-bar]').forEach(function(el){var cs=getComputedStyle(el);if(cs.position!=='fixed'&&cs.position!=='sticky')return;var x=el.getBoundingClientRect();if(x.height>0&&x.bottom>=vh-4&&x.top<vh)max=Math.max(max,vh-x.top)});b.style.bottom=max>0?'calc('+(max+14)+'px + env(safe-area-inset-bottom,0px))':''}catch(e){}}b.addEventListener('click',function(){var n=cur()==='light'?'dark':'light';r.setAttribute('data-theme',n);try{localStorage.setItem(k,n)}catch(e){}paint()});paint();document.body.appendChild(b);lift();addEventListener('resize',lift);setTimeout(lift,600)}catch(e){}});</script>`;
