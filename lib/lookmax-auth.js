...
async function recordLogin(user, provider) {
  ...
  if (isFirstLogin && updated && updated.email && (resolvedProvider === 'google' || resolvedProvider === 'email' || resolvedProvider === 'password')) {
    try {
    ...
    // Activity feed: post parent signup message on first self-serve sign-in.
    // Fire-and-forget — never blocks login, never throws.
    require('./activity-feed').userSignedUp(updated).catch(() => {}); // eslint-disable-line global-require
  }
