...
/** Set of phone numbers allowed under `allowlist` (ADMIN_PHONE + extras). */
...
/** Set of email addresses allowed under `allowlist` (ADMIN_EMAIL + extras). */
function allowedEmails() {
  const list = [];
  if (process.env.ADMIN_EMAIL) list.push(process.env.ADMIN_EMAIL);
  const extra = process.env.EMAIL_ALLOWLIST || '';
  extra.split(',').forEach((e) => {
    if (e.trim()) list.push(e.trim());
  });
