...
(function () {
  ...
  var CSS = [
    ...
    '.mcfe__canvas{max-width:340px;width:100%;height:auto;border-radius:var(--mc-r-3,14px);border:1px solid var(--mc-line,rgba(255,255,255,.10));box-shadow:0 12px 40px rgba(0,0,0,.45)}',
    '.mcfe__grid{display:grid;grid-template-columns:1fr 1fr;gap:var(--mc-sp-3,12px)}',
    '.mcfe__card{border:1px solid var(--mc-line,rgba(255,255,255,.10));border-radius:var(--mc-r-3,14px);background:var(--mc-near-black,#0c0c0e);padding:var(--mc-sp-4,18px);transition:border-color .25s ease}',
    '.mcfe__card:hover{border-color:var(--mc-silver-faint,#5a5a5a)}',
    ...
    '.mcfe__bar i{display:block;height:100%;background:linear-gradient(90deg,var(--mc-silver-dim,#8a8a8a),var(--mc-silver-bright,#e8e8e8))}',
    '.mcfe__rows div{display:flex;justify-content:space-between;font-size:12.5px;color:var(--mc-silver-dim,#8a8a8a);padding:7px 0}',
    '.mcfe__rows div + div{border-top:1px solid var(--mc-line,rgba(255,255,255,.08))}',
